...
fn test_sparse_batch_encoding() {
    ...
    for (i, emb) in batch_embeddings.iter().enumerate() {
        assert_eq!(
            ...
            "Embedding {} should have vocab size 30522",
            i
        );
        assert_eq!(
            emb.text, texts[i],
        ...
        assert!(
            nnz > 10,
            "Embedding {} should have >10 non-zero values, got {}",
            i,
            nnz
        );
    }
...
fn test_sparse_batch_vs_sequential_consistency() {
    ...
    for (i, (seq_emb, batch_emb)) in sequential.iter().zip(batch.iter()).enumerate() {
        ...
        assert_eq!(
            seq_emb.nnz(),
            batch_emb.nnz(),
            "Embedding {} should have same nnz in batch vs sequential",
            i
        );
...
fn test_sparse_similarity_semantic() {
    ...
    assert!(
        sim_high > 0.0,
        "Similar texts should have positive similarity (got {})",
        sim_high
    );
}
...
fn test_sparse_interpretability() {
    ...
    for (idx, weight) in &emb.weights {
        assert!(
            idx < &30522,
            "Vocab index {} out of bounds (max 30521)",
            idx
        );
...
fn test_sparse_weights_magnitude() {
    ...
    assert!(
        min_weight > 0.0,
        "Min weight should be positive, got {}",
        min_weight
    );
