//! Integration tests for `CandleDenseEncoder`

use candle_core::Device;
use tessera::core::{DenseEncoder, PoolingStrategy, Tokenizer};
use tessera::encoding::dense::CandleDenseEncoder;
use tessera::models::ModelConfig;
use tessera::ResourcePolicy;

#[test]
#[ignore = "requires remote model artifacts"]
fn test_dense_encoder_single() {
    let config = ModelConfig::from_registry("bge-base-en-v1.5").expect("Failed to load config");

    let device = Device::Cpu;
    let encoder = CandleDenseEncoder::new(config, device).expect("Failed to create encoder");

    // Test single encoding
    let text = "Machine learning is amazing";
    let embedding = encoder.encode(text).expect("Failed to encode");

    // Verify embedding properties
    assert_eq!(embedding.dim(), 768);
    assert_eq!(embedding.text, text);

    // Verify embedding is not all zeros
    let sum: f32 = embedding.embedding.iter().sum();
    assert!(sum.abs() > 0.0, "Embedding should not be all zeros");
}

#[test]
#[ignore = "requires remote model artifacts"]
fn test_dense_encoder_batch() {
    let config = ModelConfig::from_registry("bge-base-en-v1.5").expect("Failed to load config");

    let device = Device::Cpu;
    let encoder = CandleDenseEncoder::new(config, device).expect("Failed to create encoder");

    // Test batch encoding
    let texts = ["First document", "Second document", "Third document"];
    let embeddings = encoder
        .encode_batch(
            &texts
                .iter()
                .map(std::convert::AsRef::as_ref)
                .collect::<Vec<_>>(),
        )
        .expect("Failed to batch encode");

    // Verify batch results
    assert_eq!(embeddings.len(), 3);

    for (i, embedding) in embeddings.iter().enumerate() {
        assert_eq!(embedding.dim(), 768);
        assert_eq!(embedding.text, texts[i]);

        let sum: f32 = embedding.embedding.iter().sum();
        assert!(sum.abs() > 0.0, "Embedding should not be all zeros");
    }
}

#[test]
#[ignore = "requires remote model artifacts"]
fn test_dense_encoder_normalization() {
    // Load config with normalization enabled
    let config = ModelConfig::from_registry("bge-base-en-v1.5").expect("Failed to load config");

    let device = Device::Cpu;
    let encoder = CandleDenseEncoder::new(config, device).expect("Failed to create encoder");

    let text = "Test normalization";
    let embedding = encoder.encode(text).expect("Failed to encode");

    // BGE models use normalization by default
    // Compute L2 norm
    let norm: f32 = embedding
        .embedding
        .iter()
        .map(|x| x * x)
        .sum::<f32>()
        .sqrt();

    // Check if normalized (norm should be close to 1.0)
    assert!(
        (norm - 1.0).abs() < 0.01,
        "Embedding should be normalized, got norm {norm}"
    );
}

#[test]
#[ignore = "requires remote model artifacts"]
fn test_dense_encoder_pooling_strategy() {
    let config = ModelConfig::from_registry("bge-base-en-v1.5").expect("Failed to load config");

    let device = Device::Cpu;
    let encoder = CandleDenseEncoder::new(config, device).expect("Failed to create encoder");

    // Verify pooling strategy
    assert_eq!(encoder.pooling_strategy(), PoolingStrategy::Cls);

    // Verify embedding dimension
    assert_eq!(encoder.embedding_dim(), 768);
}

#[test]
fn test_dense_encoder_requires_pooling() {
    let mut config = ModelConfig::from_registry("bge-base-en-v1.5").expect("Failed to load config");
    config.pooling_strategy = None;

    let device = Device::Cpu;
    let result = CandleDenseEncoder::new(config, device);

    // Should fail because pooling strategy is not set
    assert!(result.is_err());
    if let Err(err) = result {
        assert!(err.to_string().contains("pooling_strategy"));
    }
}

#[test]
#[ignore = "requires remote NomicBERT artifacts and an extended-context CPU forward"]
fn test_nomic_dense_above_default_context() {
    let config = ModelConfig::from_registry("nomic-embed-v1.5")
        .expect("Nomic should remain a runnable 8K catalog entry");
    let policy = ResourcePolicy::default()
        .with_max_sequence_tokens(1_024)
        .with_max_batch_items(1)
        .with_max_batch_tokens(1_024)
        .with_max_attention_cells(1_048_576);
    let text = std::iter::repeat_n("token", 700)
        .collect::<Vec<_>>()
        .join(" ");
    let tokenizer = Tokenizer::from_pretrained_with_policy(&config.model_name, policy)
        .expect("Nomic tokenizer should load");
    let (token_ids, _) = tokenizer
        .encode(&text, true)
        .expect("the opted-in policy should accept the extended input");
    assert!(token_ids.len() > 512 && token_ids.len() <= 1_024);

    let encoder = CandleDenseEncoder::new_with_resource_policy(config, Device::Cpu, policy)
        .expect("Nomic should load through stock Candle");

    let embedding = encoder
        .encode(&text)
        .expect("Nomic should encode beyond Tessera's 512-token safe default when opted in");

    assert_eq!(embedding.dim(), 768);
    let norm = embedding
        .embedding
        .iter()
        .map(|value| value * value)
        .sum::<f32>()
        .sqrt();
    assert!((norm - 1.0).abs() < 0.01, "embedding norm was {norm}");
}
